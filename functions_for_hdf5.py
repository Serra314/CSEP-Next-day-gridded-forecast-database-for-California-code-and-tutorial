...
def get_cumulative_forecast(dict_fore, forecast_name, start_date='', end_date='', all_dates=False):
    """
    ...
    N_total_per_bin = np.repeat(0, 391782)
...
def test_per_day(dict_fore, catalog, test_fun, plot_title = 'test', alpha_ = 0.05, plot_out = True,
                 ...
                 ylims = None, size_p = 1, box_pos = (0.55, 0.5)):
    """
    ...
    # Loop through each forecast in the dictionary
    for year in dict_fore.keys():
        for month in dict_fore[year].keys():
            for fore in dict_fore[year][month].keys():
                ...
                if tmp_cat.to_dataframe().shape[0] != 0:
                    tmp_fore.to_csv('tmp_fore_L.dat', sep='\t', header=None, index=None)
                    fore_ = csep.load_gridded_forecast('tmp_fore_L.dat', name='1')
                    os.remove('tmp_fore_L.dat')
                    test_ = test_fun(fore_, tmp_cat)
                    
                    # Store the test results for this day
                    date_fore_list.append(date_fore)
                    obs_score_per_day.append(test_.observed_statistic)
                    lowerq_per_day.append(np.quantile(test_.test_distribution, alpha_ / 2))
                    upperq_per_day.append(np.quantile(test_.test_distribution, (1 - alpha_) / 2))
                    quantile_per_day.append(test_.quantile)
    ...
    for year in dict_fore.keys():
        for month in dict_fore[year].keys():
            for fore in dict_fore[year][month].keys():
                ...
                # If there are observations for this forecast, run the N-test
                if tmp_cat.to_dataframe().shape[0] != 0:
                    tmp_fore.to_csv('tmp_fore_L.dat', sep='\t', header=None, index=None)
                    fore_ = csep.load_gridded_forecast('tmp_fore_L.dat', name='1')
                    os.remove('tmp_fore_L.dat')
                    ...
                    lowerq_per_day.append(poisson_stat.ppf(alpha_/2, fore_rate))
                    upperq_per_day.append(poisson_stat.ppf(1 - alpha_/2, fore_rate))
                    quantile_per_day.append(poisson_stat.cdf(test_.observed_statistic, fore_rate))
...
def score_diff_per_day(dict_fore1, dict_fore2, catalog, score_fun = csep.utils.stats.get_Kagan_I1_score):
    """
    ...
    # Loop through each forecast in the first model's dictionary
    for year in dict_fore1.keys():
        for month in dict_fore1[year].keys():
            # Get the keys for the second model's forecasts
            fore2_keys = dict_fore2[year][month].keys()
            
            # Loop through each forecast in the first model
            for fore in dict_fore1[year][month].keys():
                ...
                # Generate the start and end date for the forecast period
                start_date = str(date_fore.year) + '-' + str(date_fore.month) + '-' + str(date_fore.day) + ' 00:00:00.0'
                end_date = date_fore + timedelta(days=1) 
                end_date = str(end_date.year) + '-' + str(end_date.month) + '-' + str(end_date.day) + ' 00:00:00.0'
                ...
                # Get the corresponding forecast from the second model for the same date
                fore2_name = [k for k in fore2_keys if extract_file_date(k) == date_str][0]
                tmp_fore1 = dict_fore1[year][month][fore].copy()
                tmp_fore2 = dict_fore2[year][month][fore2_name].copy()
                
                # Filter the catalog for events that occurred during the forecast period
                filters = [f'origin_time >= {start_epoch}', f'origin_time < {end_epoch}']
                tmp_cat = catalog.filter(filters, in_place=False)
                
                # If there are observations for this forecast, compute the score difference
                if tmp_cat.to_dataframe().shape[0] != 0:
                    # Save the forecasts to temporary files
                    tmp_fore1.to_csv('tmp_fore1.dat', sep='\t', header=None, index=None)
                    tmp_fore2.to_csv('tmp_fore2.dat', sep='\t', header=None, index=None)
                    
                    # Load the forecasts into the CSEP format
                    fore1 = csep.load_gridded_forecast('tmp_fore1.dat', name='1')
                    fore2 = csep.load_gridded_forecast('tmp_fore2.dat', name='2')
                    
                    os.remove('tmp_fore1.dat')
                    os.remove('tmp_fore2.dat')
                    
                    # Filter the catalog by the region of the forecast
                    tmp_cat.filter_spatial(fore1.region)
                    
                    # Calculate the Kagan Information score for both models
                    K_score1 = score_fun(fore1, tmp_cat)[0]
                    K_score2 = score_fun(fore2, tmp_cat)[0]
                    
                    # Append the results to the lists
                    date_fore_list.append(date_fore)
                    score_diff.append(K_score1 - K_score2)
